...
The fixed product bar contains identity, the member drawer trigger, offline state when needed, and the account menu. Card creation is closed by default: the toolbar's 新增卡片 button, sharing a row with the filters, opens Quick Create as a dialog in the title-plus-controls layout, and the dialog stays open after a create so several cards can be entered in sequence. Its segmented mode switches between one team and all team leads; additional options cover status, description, and searchable project labels. New cards default to Inbox and appear at the top of the target lane.
